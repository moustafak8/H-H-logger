...
class Ai_response
{
    public static function generateAIResponse($text)
    {
     ...
     public static function Nutritioncoach($text){
         global $apiKey;
        $system = "You are an AI nutrition coach for a habit-tracking app. The user provides a description of a meal or food they consumed. Estimate the total calories in that meal based on typical nutritional data. Then, suggest a healthy next meal option that complements the current one, focusing on balance, nutrition, and variety. Return ONLY valid JSON (no extra text, no explanation). Output must be a single JSON object with 'estimated_calories' (integer) and 'suggestion' (string) fields.";
        $userContent = "Meal description: " . $text;
